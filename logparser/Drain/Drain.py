...
class Node:
    def __init__(self, childD=None, depth=0, digitOrtoken=None,parent=None):
        ## note childD can be different type
        if childD is None:
            childD = dict()
        self.childD = childD
        self.depth = depth
...
class LogParser:
    ...
    def addSeqToPrefixTree(self, rn, logClust):
        ...
        for token in logClust.logTemplate:
            ...
            # if log message len is shorter than this.depth,it also will append to the node which depth < this.depth
            if currentDepth >= self.depth or currentDepth > seqLen:
                if len(parentn.childD) == 0:
                    ##注意這邊把childD從dict換掉變成list
            ...
            if token not in parentn.childD:
                ...
                ## if there is digit in token ,select node which value is <*>
                else:
    ...
    #seq1 is template
    #caculate similarity between seq and template 
    ...
    #find the best match witch curseq in the left group
    ...
    #找到group之後 將這個group的template修改 一些地方要變成*
    ...
    def outputResult(self, logClustL):
        ...
        for logClust in logClustL:
            ...
            # 將每一個log做logid logtemplate 的標記 結果會放在_structured.csv裡面
    ...
    def bsfPrintTree(self,root,dep):
        que = queue.Queue(maxsize = 100)  
        que.put(root)
        curDepth = 0
        while que.empty()!=True:
            print(curDepth,"--------curDepth----------------------------")
            curLayerSize = que.qsize()
            for index in range(curLayerSize):
                curNode = que.get()
                print(curNode.digitOrtoken," ",end='')
                if curNode.parent is not None:
                    print("###parent",curNode.parent.digitOrtoken," ")
                for key ,node in curNode.childD.items():
                    #print(key," ",end='')
                    node.parent = curNode
                    que.put(node)
    ...
    def parse(self, logName):
        ...
        for idx, line in self.df_log.iterrows():
            ...
            #Add the new log message to the existing cluster
                ...
                # check whether need to update template
        ...
        #self.bsfPrintTree(rootNode,self.depth)
    ...
    #根據user 提供的regex去把log content裡面的像是IP等等先換成星號,這個步驟其實也可以不做,因為這個功能因該要是這個程想要
    #達成的目的
